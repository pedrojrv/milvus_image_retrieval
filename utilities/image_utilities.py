...
def show_query_results(inventory, search_results, new=False):
...
def get_similar_images(client, collection_name, image, top_k=6, nprobe=16, plot=False, inventory=None):
        ...
        if plot:
            if isinstance(image, str):
                if top_k >= 6:
                    ...
                    show_query_results(inventory, results, new=True)
                else:
